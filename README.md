...
<img src="public/pip-mascot.png" alt="Pip, the wide-eyed AI companion" width="170">

# Pip

**Your private team of customizable AI characters, hosted on your Mac.**
...
> **This ZIP is the private Pip fork.** Use [SETUP.md](SETUP.md) and `SETUP-MAC.command` to build it. The upstream OpenMausBot release links preserved below are project history and do not contain the Pip PWA/onboarding changes.
...
This checkout includes an optional, owner-paired PWA served by the companion gateway. The Mac remains the single local source of truth; iPhone, Android, Windows, and macOS browsers use the same responsive UI and live SSE state through a private Tailscale Serve HTTPS origin.

- Browser credentials are HttpOnly, SameSite=Strict, and never enter React or the offline cache.
- Unknown remote API routes fail closed. Full PWA management and computer view are separate per-device permissions enabled only from the Mac.
- `Dockerfile` and `compose.yaml` support headless chat/orchestration, but macOS Accessibility, Screen Recording, Apple speech, and host control remain native-only.
- The service worker caches only the application shell, never API responses, chats, images, audio, or secrets.

For a first-time Mac install, start with [SETUP.md](SETUP.md). For technical detail, see [PWA and Tailscale setup](docs/PWA-TAILSCALE.md), [feature coverage](docs/FEATURE-PARITY.md), [architecture](docs/ARCHITECTURE.md), the [threat model](docs/THREAT-MODEL.md), the [security red-team record](docs/SECURITY-RED-TEAM.md), and the [production-readiness gates](docs/PRODUCTION-READINESS.md).
