...
export function createControlServer(options: ControlOptions): Server {
  return createServer((req, res) => {
    ...
    const administration = path.match(/^\/devices\/([\w-]+)\/administration$/);
    if (administration && (method === "POST" || method === "DELETE")) {
      try {
        if (!options.devices.setAdministrationAccess(administration[1], method === "POST")) {
          return json(res, 404, { error: "no such device" });
        }
      } catch {
        return json(res, 500, { error: "could not save administration access" });
      }
      return json(res, 200, companionState(options));
    }
...
function render(s) {
  ...
  el("devices").innerHTML =
    ...
    (s.devices.length
      ? "<ul>" + s.devices.map((d) =>
          ...
          "<button data-admin='" + esc(d.id) + "' data-allowed='" + (d.administrationAccess ? "1" : "0") + "'>" +
          (d.administrationAccess ? "Administration on" : "Allow administration") + "</button></div>" +
  ...
  for (const b of document.querySelectorAll("[data-admin]")) {
    b.addEventListener("click", async () => {
      const method = b.dataset.allowed === "1" ? "DELETE" : "POST";
      render(await api("/devices/" + b.dataset.admin + "/administration", method));
    });
  }
  if (s.pairing) {
    const tick = () => {
