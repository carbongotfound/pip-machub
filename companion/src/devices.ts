...
export interface DeviceRecord {
  ...
  /** Remote changes to credentials, deletion, webhooks, imports, and host
   * configuration. Pairing alone never grants this capability. */
...
function normalizeDevice(record: Partial<DeviceRecord> & { id: string; tokenHash: string }): DeviceRecord {
  ...
  return {
    ...
    administrationAccess: record.administrationAccess === true,
  };
}
...
export class DeviceRegistry {
  ...
  /** Grant or remove the high-risk browser administration surface. Kept
   * separate from cloud desktop viewing so neither permission implies the
   * other and a lost phone can be reduced to ordinary chat access. */
  setAdministrationAccess(id: string, allowed: boolean): boolean {
    const device = this.devices.find((candidate) => candidate.id === id);
    if (!device) return false;
    const previous = device.administrationAccess;
    device.administrationAccess = allowed;
    try {
      this.persist();
    } catch (error) {
      device.administrationAccess = previous;
      throw error;
    }
    return true;
  }
