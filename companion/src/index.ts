...
const PWA_DIR = process.env.OMB_PWA_DIR?.trim() || undefined;
...
const machineName = (): string => cachedName || "Pip";
...
async function refreshMachineName(): Promise<void> {
  ...
  try {
    ...
    /* not up, or no profile — "Pip" is a fine thing to be called */
  }
}
...
const proxy = createProxyHandler({
    ...
    pwaDir: PWA_DIR,
    trustPwaProxy: process.env.OMB_PWA_TRUST_PROXY === "1",
  });
...
async function main(): Promise<void> {
  ...
  if (PWA_DIR) console.log(`PWA assets ${PWA_DIR}`);
