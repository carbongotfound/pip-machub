 ...
 * itself.
 */
export function isOnLink(from: string, interfaces?: ReturnType<typeof networkInterfaces>): boolean {
  const source = toIpv4(from);
  if (source === null) return false;
  let table: ReturnType<typeof networkInterfaces>;
  try {
    table = interfaces ?? networkInterfaces();
  } catch {
    // Discovery is optional. Sandboxes that hide the interface table should
    // receive unicast-safe behavior, not an uncaught exception in the UDP
    // message handler.
    return false;
  }
...
export function dnsLabel(text: string, fallback = "Pip"): string {
  let label = text
    .replace(/[\u0000-\u001f\u007f]/g, "")
    .replace(/\./g, " ")
    .trim();
