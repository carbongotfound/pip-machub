...
import { DeviceRateLimiter } from "./rate-limit.ts";
import {
...
export interface ProxyOptions {
  ...
  /** Built Vite output. Absent keeps the native-only companion behavior. */
  pwaDir?: string;
  /** Trust HTTPS forwarding headers from a non-loopback reverse proxy. The
   * native Tailscale Serve path is loopback and does not need this. */
  trustPwaProxy?: boolean;
}
...
/** Enough for the harness's 10 MB decoded image upload plus base64/JSON
 * framing, while still bounding chunked requests from an authenticated but
 * compromised device. */
...
/** The device-facing handler: serve the optional PWA shell, validate browser
 * same-origin requests or native bearer requests, apply the allowlist and
 * capabilities, then replay accepted API calls to the loopback harness. */
export function createProxyHandler(options: ProxyOptions) {
  ...
  return function handle(req: IncomingMessage, res: ServerResponse): void {
    ...
    if (servePwaAsset(req, res, options.pwaDir)) return;

    const pwa = path === "/api/pwa/session" || path === "/api/pwa/logout" || isPwaRequest(req);
    const pwaCheck = pwa ? validatePwaRequest(req, { trustProxy: options.trustPwaProxy }) : null;
    if (pwaCheck && !pwaCheck.ok) {
      return sendJson(res, pwaCheck.status, { error: pwaCheck.error });
    }

    // A browser can authenticate only with the HttpOnly cookie minted by a
    // same-origin pairing. Native clients keep the existing bearer contract.
    const token = pwa ? pwaSessionToken(req) : bearerToken(req.headers.authorization);
    const device = options.authenticate(token);

    if (pwa && method === "GET" && path === "/api/pwa/session") {
      return sendJson(res, 200, {
        ...
        device: device
          ? {
              ...
              administrationAccess: device.administrationAccess === true,
              cloudDesktopAccess: device.cloudDesktopAccess,
            }
          : null,
      });
    }
    if (pwa && method === "POST" && path === "/api/pwa/logout") {
      return sendJson(
        res,
        200,
        { authenticated: false },
        { "set-cookie": clearPwaSessionCookie(Boolean(pwaCheck?.secure)) },
      );
    }
    ...
    const denial = denyReason({
      ...
      authenticated: Boolean(device),
      administrative: device?.administrationAccess === true,
    });
    if (denial) return sendJson(res, denial.status, { error: denial.error });

    if (device?.id && path !== "/api/events") {
      const decision = rateLimiter.check(device.id, method);
      if (!decision.allowed) {
        return sendJson(
          res,
          429,
          { error: "this device is sending requests too quickly — try again shortly" },
    ...
    if (isCloudDesktopJoin(method, path) && !device?.cloudDesktopAccess) {
      return sendJson(res, 403, {
        error: "cloud desktop access is off for this phone — enable it in Pip → Settings → Phone",
      });
    }
    ...
    if (method === "POST" && path === "/api/pair") {
      readJson(req).then(
        (body) => {
          ...
          const response: Omit<typeof result, "token"> & {
            token?: string;
            serverName: string;
            hosts?: string[];
            endpoints?: CompanionEndpoint[];
          ...
          if (!pwa) response.token = result.token;
          ...
          return sendJson(
            ...
            response,
            pwa
              ? { "set-cookie": pwaSessionCookie(result.token, Boolean(pwaCheck?.secure)) }
              : {},
          );
        },
      ...
      (harness) => {
        ...
        if (method === "GET" && path === "/api/health") {
          ...
          const fail = () => {
            if (finished) return;
            finished = true;
            sendJson(res, 502, { error: "Pip is not ready on this computer" });
          };
        ...
        if (isStream) {
          ...
          if (tracksDeviceConnection && currentDevice?.id !== device?.id) {
            ...
            return sendJson(res, 401, {
              error: "pair this device from Phone settings in Pip on your computer",
            });
          }
          const disconnect = () => {
        ...
        harness.on("data", (chunk: Buffer) => {
          ...
          if (size > MAX_JSON_BODY_BYTES) {
            ...
            else sendJson(res, 502, { error: "the response from Pip was too large" });
            return;
          }
          chunks.push(chunk);
        });
    ...
    upstream.on("error", () => {
      ...
      sendJson(
        ...
        timedOut ? 504 : 502,
        timedOut
          ? { error: "Pip did not respond" }
          : { error: "Pip is not running on this computer" },
      );
    });
    let requestBytes = 0;
    ...
    req.on("data", (chunk: Buffer) => {
      ...
      if (requestBytes <= MAX_REQUEST_BODY_BYTES) return;
