...
export interface RouteRequest {
  ...
  /** Explicit per-device permission granted from the loopback control plane. */
...
/** Desktop-equivalent operations. These are deliberately enumerated rather
 * than turning administration into `/api/**`: an upstream route added later
 * remains unreachable until it is reviewed here. */
const ADMIN_ALLOWED: ReadonlyArray<{ method: string; path: RegExp }> = [
  { method: "GET", path: /^\/api\/team-map$/ },
  { method: "GET", path: /^\/api\/section-context$/ },
  { method: "PUT", path: /^\/api\/section-context$/ },
  ...
  { method: "POST", path: /^\/api\/teams\/export$/ },
  { method: "POST", path: /^\/api\/teams\/import$/ },
  { method: "GET", path: /^\/api\/teams\/scout$/ },
  { method: "GET", path: /^\/api\/teams\/scout\/directory$/ },
  { method: "GET", path: /^\/api\/team-library\/catalog$/ },
  { method: "GET", path: /^\/api\/team-library\/teams\/[a-z0-9][a-z0-9-]*$/ },
  { method: "POST", path: /^\/api\/team-library\/github$/ },

  { method: "PATCH", path: /^\/api\/groups\/[\w-]+$/ },
  { method: "DELETE", path: /^\/api\/groups\/[\w-]+$/ },
  { method: "POST", path: /^\/api\/groups\/[\w-]+\/setup$/ },
  { method: "POST", path: /^\/api\/groups\/[\w-]+\/interrupt$/ },

  { method: "GET", path: /^\/api\/threads\/[\w-]+\/events$/ },
  ...
  { method: "PUT", path: /^\/api\/bots\/[\w-]+\/skills\/[a-z0-9-]+$/ },
  { method: "DELETE", path: /^\/api\/bots\/[\w-]+\/skills\/[a-z0-9-]+$/ },
  { method: "GET", path: /^\/api\/bots\/[\w-]+\/memory$/ },
  { method: "PUT", path: /^\/api\/bots\/[\w-]+\/memory$/ },
  { method: "DELETE", path: /^\/api\/bots\/[\w-]+\/memory\/topics\/[^/]+$/ },
  { method: "GET", path: /^\/api\/bots\/[\w-]+\/checkpoints$/ },
  { method: "POST", path: /^\/api\/bots\/[\w-]+\/checkpoints\/restore$/ },
  { method: "POST", path: /^\/api\/bots\/[\w-]+\/cards\/[\w-]+$/ },

  { method: "GET", path: /^\/api\/local-computer$/ },
  { method: "POST", path: /^\/api\/local-computer\/(?:pull|run|start|stop|remove|interrupt|screenshot)$/ },
  { method: "GET", path: /^\/api\/bots\/[\w-]+\/local-computer$/ },
  { method: "POST", path: /^\/api\/bots\/[\w-]+\/local-computer\/(?:run|stop|remove|screenshot)$/ },

  { method: "GET", path: /^\/api\/cli-candidates$/ },
  ...
  { method: "DELETE", path: /^\/api\/connectors\/[\w-]+$/ },

  { method: "POST", path: /^\/api\/bots\/[\w-]+\/secret-cards\/[\w-]+\/(?:provided|resume|dismiss)$/ },
  { method: "POST", path: /^\/api\/bots\/[\w-]+\/connector-cards\/[\w-]+\/(?:authorize|status|resume|dismiss)$/ },
  ...
  { method: "POST", path: /^\/api\/bots\/[\w-]+\/computer\/viewer-close$/ },
  { method: "POST", path: /^\/api\/bots\/[\w-]+\/computer\/(?:provision|sleep|exec|screenshot|remove)$/ },
];
...
export function denyReason({ path, method, authenticated, administrative = false }: RouteRequest): Denial | null {
  ...
  if (!authenticated) {
    return { status: 401, error: "pair this device from Phone settings in Pip on your computer" };
  }

  if (ALLOWED.some((route) => route.method === method && route.path.test(path))) return null;
  if (administrative && ADMIN_ALLOWED.some((route) => route.method === method && route.path.test(path))) {
    return null;
  }
