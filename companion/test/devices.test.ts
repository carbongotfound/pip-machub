...
describe("DeviceRegistry", () => {
  ...
  it("keeps administration off until enabled for that device", () => {
    const registry = new DeviceRegistry();
    const pairing = registry.openPairing();
    const result = registry.redeem(pairing.token, "Browser PWA");
    expect("error" in result).toBe(false);
    if ("error" in result) return;
    expect(registry.authenticate(result.token)?.administrationAccess).toBe(false);
    expect(registry.setAdministrationAccess(result.device.id, true)).toBe(true);
    expect(registry.authenticate(result.token)?.administrationAccess).toBe(true);
    expect(new DeviceRegistry().authenticate(result.token)?.administrationAccess).toBe(true);
  });
