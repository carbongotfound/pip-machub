...
describe("explicit administration capability", () => {
  it("opens reviewed desktop routes only when separately granted", () => {
    expect(ask("PUT", "/api/config")?.status).toBe(403);
    expect(denyReason({ method: "PUT", path: "/api/config", authenticated: true, administrative: true })).toBeNull();
    expect(denyReason({ method: "DELETE", path: "/api/bots/bot-1", authenticated: true, administrative: true })).toBeNull();
  });

  it("still denies internal and unknown future routes", () => {
    expect(denyReason({ method: "POST", path: "/api/internal/delegate-bot", authenticated: true, administrative: true })?.status).toBe(404);
    expect(denyReason({ method: "POST", path: "/api/new-upstream-admin", authenticated: true, administrative: true })?.status).toBe(404);
  });
});
