...
async function start({ resourcesPath, harnessPort, hostedUrl = null, log }) {
  ...
  const pwaDir = app.isPackaged ? path.join(resourcesPath, "ui") : path.join(app.getAppPath(), "dist");
  if (fs.existsSync(path.join(pwaDir, "index.html"))) childEnvironment.OMB_PWA_DIR = pwaDir;
  else delete childEnvironment.OMB_PWA_DIR;
...
/** Enable or remove remote administration for one paired device. */
export async function companionAdministrationAccess(deviceId, allowed) {
  if (!proc) return companionState();
  if (!/^[\w-]{1,64}$/.test(String(deviceId ?? ""))) return companionState();
  await control(allowed ? "POST" : "DELETE", `/devices/${deviceId}/administration`).catch(() => {});
  return companionState();
}
