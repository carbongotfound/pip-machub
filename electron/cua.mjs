...
//    EmbeddedCuaDriverHost so TCC grants attribute to Pip and the
//    driver inherits them. One prompt, named Pip, out of the box.
...
async function attachStandalone() {
  ...
  if (!(await socketAlive(STANDALONE_SOCKET))) {
    ...
    // machine already granted — instead of the freshly signed Pip.
...
async function startEmbedded(binary) {
  ...
  // Pip rather than to a terminal or helper process.
  ...
  if (!sdk.hasRequiredMacOSPermissions(permissionStatus)) {
    ...
    throw new Error(`${missing || "macOS permissions"} required; grant access in System Settings and restart Pip`);
  }
