...
if (!app.requestSingleInstanceLock()) {
  console.log("[desktop] Pip is already running — focusing that window");
  process.exit(0);
}
...
// goes to server.log in the OS log dir (~/Library/Logs/Pip on macOS,
// Console.app-visible; %APPDATA%\Pip\logs on Windows), which is also
...
function buildErrorPage({ allPortsOccupied }) {
  ...
  const reason = allPortsOccupied
    ? "Every Pip port answered health checks from another process — likely a second copy of the app, or another program on ports 8799–28799. Quit that program, then quit and reopen Pip."
    : "The background server didn't come up in time — this is usually slow startup, not a port conflict. Quit and reopen Pip.";
  return (
...
function openDesktopViewer(owner, rawUrl, rawTitle, contextId) {
  if (!owner || owner.isDestroyed()) throw new Error("The Pip window is unavailable");
...
// Bots hand users files as markdown links to paths inside the Pip
...
// live desktop inside Pip instead of sending the person to a browser.
...
ipcMain.handle("companion:administration", (_event, deviceId, allowed) =>
  companionAdministrationAccess(deviceId, Boolean(allowed)).then(() => desktopCompanionState()),
);
