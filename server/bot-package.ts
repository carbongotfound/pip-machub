...
import { MASCOT_ACCESSORY_NAMES, MASCOT_BODY_NAMES, MASCOT_EYE_NAMES } from "../shared/mascot-profile.ts";
...
export function packageAgentAsMember(agent: BotPackageAgent): TeamManifestMember {
  ...
  if (agent.appearance.mascotBody) appearance.mascotBody = agent.appearance.mascotBody;
  if (agent.appearance.mascotEyes) appearance.mascotEyes = agent.appearance.mascotEyes;
  if (agent.appearance.mascotAccessory) appearance.mascotAccessory = agent.appearance.mascotAccessory;
  return {
    key: agent.key,
    name: agent.name,
    title: agent.title ?? "",
