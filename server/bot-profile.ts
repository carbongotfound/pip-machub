...
const profilePatchSchema = z.object({
  ...
  mascotExpression: z
    .string({ error: "mascotExpression must be a string" })
    .max(80, { error: "mascotExpression must be at most 80 characters" })
    .nullable()
    .optional(),
  mascotBody: z.enum(MASCOT_BODY_NAMES, { error: "mascotBody is not supported" }).optional(),
  mascotEyes: z.enum(MASCOT_EYE_NAMES, { error: "mascotEyes is not supported" }).optional(),
  mascotAccessory: z.enum(MASCOT_ACCESSORY_NAMES, { error: "mascotAccessory is not supported" }).optional(),
  avatarUrl: z
...
export type BotProfilePatch = Partial<
  Pick<
    BotRecord,
    "name" | "title" | "description" | "notifications" | "color" | "mascotExpression" | "mascotBody" | "mascotEyes" | "mascotAccessory" | "avatarUrl" | "avatarCrop" | "voice" | "speakReplies"
  >
>;
