// Pip server — the harness host. Clients hold no transports
...
import { buildTurnContext, engineIsFresh, selectTranscriptReplay } from "./turn-context.ts";
...
async function startTurn(
  ...
  const transcript = selectTranscriptReplay(activeMessages
  ...
  const persona = [
    `You are ${bot.name}, a personal bot in Pip.`,
  ...
  void (async () => {
    try {
      ...
      if (wants === "vm") {
        ...
        if (!cua) throw new Error("CUA Driver is not ready for this computer — check permissions and restart Pip");
...
async function cloudRoutineReadiness(): Promise<{ ready: boolean; reason?: string }> {
  ...
  if (!instance) {
    return { ready: false, reason: "The Cloud VM runner is unavailable. Restart Pip and try again." };
  }
  try {
...
async function runGroupMemberTurn(
  ...
  const roster = group.memberIds
    ...
    .join(", ");
  const system = [
    `You are ${bot.name}, a bot in the room "${group.name}" in Pip.`,
    bot.title && `Role: ${bot.title}.`,
    bot.description && `About: ${bot.description}`,
...
function dispatchConnectorResume(entry: { botId: string; threadId: string; resumeKey: string; labels: string[] }) {
  ...
  const prompt = `Pip connection update: the user securely connected ${names}. Continue the task that paused for this connection. Do not ask them to connect it again.`;
  if (owner.bot.busy) {
...
function dispatchSecretResume(entry: SecretResumeEntry) {
  ...
  const prompt =
    entry.outcome === "provided"
      ? `Pip credential update: the user securely provided ${entry.label}. Continue the task that paused for it. You do not receive the secret and must not ask them to paste it into chat.`
      : `Pip credential update: the user declined to provide ${entry.label}. Continue without it if possible, or briefly explain the limitation. Do not ask them to paste it into chat.`;
  if (owner.bot.busy) {
  ...
  try {
    ...
    if (path.startsWith("/api/internal/")) {
      ...
      if (method === "POST" && path === "/api/internal/ask-bot") {
        ...
        const prefixed = `[Message from @${currentFrom.name}, another bot in this Pip workspace. Reply to them.]\n\n${message}`;
    ...
    // relay never has to expose the rest of Pip's control surface.
    ...
    if (m && method === "PATCH") {
      ...
      for (const key of ["unread", "computer", "cloudBackend", "pinned", "hidden"] as const) {
