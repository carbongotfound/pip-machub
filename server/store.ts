...
export class Store {
  ...
  createBot(
    profile: Partial<
      Pick<BotRecord, "name" | "title" | "description" | "color" | "mascotExpression" | "mascotBody" | "mascotEyes" | "mascotAccessory" | "modelSelection" | "section">
    > = {},
    opts: {
    ...
    const bot: BotRecord = {
      ...
      mascotBody: profile.mascotBody ?? MASCOT_BODY_NAMES[this.bots.length % MASCOT_BODY_NAMES.length],
      mascotEyes: profile.mascotEyes ?? MASCOT_EYE_NAMES[this.bots.length % MASCOT_EYE_NAMES.length],
      mascotAccessory: profile.mascotAccessory ?? "none",
      unread: false,
