...
const memberSchema = z.object({
  ...
  appearance: z.object({
    ...
    mascotBody: z.enum(MASCOT_BODY_NAMES).optional(),
    mascotEyes: z.enum(MASCOT_EYE_NAMES).optional(),
    mascotAccessory: z.enum(MASCOT_ACCESSORY_NAMES).optional(),
  }),
});
...
export function parseTeamManifest(value: TeamManifestInput): ParsedTeamManifest {
  ...
  const members = parsed.data.team.members.map((member): TeamManifestMember => {
    ...
    if (member.appearance.mascotEyes) appearance.mascotEyes = member.appearance.mascotEyes;
    if (member.appearance.mascotAccessory) appearance.mascotAccessory = member.appearance.mascotAccessory;
    return {
      key: member.key,
      name: member.name,
      title: member.title ?? "",
...
export function importedMemberProfile(
  ...
  if (member.appearance.mascotBody) profile.mascotBody = member.appearance.mascotBody;
  if (member.appearance.mascotEyes) profile.mascotEyes = member.appearance.mascotEyes;
  if (member.appearance.mascotAccessory) profile.mascotAccessory = member.appearance.mascotAccessory;
  return profile;
}
...
export function createTeamManifest(team: ExportableTeam, bots: ExportableBot[]): TeamManifestV2 {
  ...
  const members = team.memberIds.map((id, index): TeamManifestMember => {
    ...
    if (bot.mascotEyes) appearance.mascotEyes = bot.mascotEyes;
    if (bot.mascotAccessory) appearance.mascotAccessory = bot.mascotAccessory;
    return {
      key,
      name: bot.name,
      title: bot.title,
