...
describe("selectTranscriptReplay", () => {
  it("returns short transcripts byte-for-byte and reference-for-reference", () => {
    const messages = [
      { role: "user" as const, text: "Build the Mac hub" },
      { role: "assistant" as const, text: "Starting" },
    ];
    expect(selectTranscriptReplay(messages)).toBe(messages);
  });

  it("keeps the initial task and recent working set within the same message cap", () => {
    const messages = Array.from({ length: 60 }, (_, index) => ({
      role: index % 2 ? ("assistant" as const) : ("user" as const),
      text: index === 0 ? "Never remove local-only storage" : `message ${index}`,
    }));
    const selected = selectTranscriptReplay(messages, 10);
    expect(selected).toHaveLength(10);
    expect(selected[0].text).toBe("Never remove local-only storage");
    expect(selected[1].text).toContain("canonical transcript");
    expect(selected.at(-1)?.text).toBe("message 59");
  });
});
