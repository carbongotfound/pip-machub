...
export type ReplayMessage = { role: "user" | "assistant"; text: string };
export const TRANSCRIPT_REPLAY_MESSAGES = 40;

/** Keep the active task anchor as well as the recent working set when a long
 * thread must be replayed into a fresh engine. Upstream kept only the last 40
 * messages, which was token-bounded but silently discarded the request that
 * often defines the whole project. This uses the same maximum count, keeps
 * canonical storage untouched, and tells the agent where the omitted middle
 * can be recovered through its bounded transcript search tool. */
export function selectTranscriptReplay(
  messages: ReplayMessage[],
  limit = TRANSCRIPT_REPLAY_MESSAGES,
): ReplayMessage[] {
  if (messages.length <= limit) return messages;
  const safeLimit = Math.max(4, Math.floor(limit));
  const firstUser = messages.find((message) => message.role === "user") ?? messages[0];
  const tail = messages.slice(-(safeLimit - 2));
  if (tail.includes(firstUser)) return messages.slice(-safeLimit);
  const omitted = messages.length - tail.length - 1;
  return [
    firstUser,
    {
      role: "assistant",
      text:
        `[OpenMausBot context note: ${omitted} settled messages between the initial task and the recent working set are not replayed here to avoid repeated tokens. ` +
        "They remain unchanged in the canonical transcript; use transcript search before relying on an uncertain earlier detail.]",
    },
    ...tail,
  ];
}
