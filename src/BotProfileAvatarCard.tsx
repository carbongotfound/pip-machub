...
type AvatarPatch = Partial<
  Pick<Bot, "avatarCrop" | "avatarUrl" | "color" | "mascotExpression" | "mascotBody" | "mascotEyes" | "mascotAccessory">
>;
...
const BODY_LABEL = {
  bean: "Bean", sprout: "Sprout", drop: "Drop", flop: "Flop", puff: "Puff", bolt: "Bolt",
} satisfies Record<MascotBody, string>;
const EYE_LABEL = {
  bright: "Bright", soft: "Soft", focused: "Focused", wide: "Wide",
} satisfies Record<MascotEyes, string>;
const ACCESSORY_LABEL = {
  none: "None", leaf: "Leaf", headphones: "Headphones", crown: "Crown", beanie: "Beanie", glasses: "Glasses",
            ...
            <div className="grid grid-cols-3 gap-2">
              {MASCOT_BODY_NAMES.map((body) => (
                <button
                  key={body}
                  type="button"
                  aria-pressed={(bot.mascotBody ?? "bean") === body}
                  onClick={() => onPatch({ mascotBody: body })}
                  className={cn(
                    "flex h-[68px] flex-col items-center justify-center gap-0.5 rounded-xl bg-inset text-[10.5px] text-ink-secondary transition hover:-translate-y-0.5 hover:bg-control hover:text-ink focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus",
                    (bot.mascotBody ?? "bean") === body && "text-ink ring-2 ring-accent-border",
                  )}
                >
                  <MausAvatar color={bot.color} body={body} eyes={bot.mascotEyes} state="happy" size={40} animated={false} />
                  {BODY_LABEL[body]}
                </button>
              ))}
            </div>
            ...
            <div className="grid grid-cols-4 gap-2">
              {MASCOT_EYE_NAMES.map((eyes) => (
                <button
                  key={eyes}
                  type="button"
                  aria-pressed={(bot.mascotEyes ?? "bright") === eyes}
                  onClick={() => onPatch({ mascotEyes: eyes })}
                  className={cn(
                    "flex h-[64px] flex-col items-center justify-center gap-0.5 rounded-xl bg-inset text-[9.5px] text-ink-secondary transition hover:bg-control hover:text-ink focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus",
                    (bot.mascotEyes ?? "bright") === eyes && "text-ink ring-2 ring-accent-border",
                  )}
                >
                  <MausAvatar color={bot.color} body={bot.mascotBody} eyes={eyes} state="idle" size={37} animated={false} />
                  {EYE_LABEL[eyes]}
                </button>
              ))}
            </div>
            ...
            <div className="grid grid-cols-3 gap-2">
              {MASCOT_ACCESSORY_NAMES.map((accessory) => (
                <button
                  key={accessory}
                  type="button"
                  aria-pressed={(bot.mascotAccessory ?? "none") === accessory}
                  onClick={() => onPatch({ mascotAccessory: accessory })}
                  className={cn(
                    "flex h-[68px] flex-col items-center justify-center gap-0.5 rounded-xl bg-inset text-[10px] text-ink-secondary transition hover:bg-control hover:text-ink focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus",
                    (bot.mascotAccessory ?? "none") === accessory && "text-ink ring-2 ring-accent-border",
                  )}
                >
                  <MausAvatar
                    color={bot.color}
