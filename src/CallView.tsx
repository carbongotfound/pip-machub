...
export function CallTargetButton({
  ...
  const reason = !capabilitiesReady
    ...
    : !capabilities.dictation.available
      ? "Calls require Pip for macOS because speech recognition runs on-device."
      : !window.ogb?.speechStart
        ? "The speech service is unavailable in this app build. Restart or update Pip."
        : !configured
...
function Call({ bot }: { bot: Bot }) {
  ...
  return (
    <div className="absolute inset-0 z-30 flex flex-col items-center justify-center gap-6 bg-app/95 backdrop-blur-sm">
      ...
      <MausAvatar color={bot.color} body={bot.mascotBody} eyes={bot.mascotEyes} accessory={bot.mascotAccessory} state={mascotState} size={220} animated trackPointer />
