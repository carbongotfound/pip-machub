...
export function CompanionSection({ profileEmail = "" }: { profileEmail?: string }) {
  ...
  if (!companionBridge()) {
    return (
      <Card
        title="Use Pip from your phone"
        subtitle="Open Settings in the Pip desktop app to set up a phone."
      />
    );
  }
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <Card
        title="Paired phones"
        subtitle={pairedCount ? "Manage the phones that can use this Pip." : "No phones are paired yet."}
      >
        {pairedCount > 0 && (
            ...
            {state.devices.map((device) => (
              <li key={device.id} className="rounded-xl bg-inset px-3 py-3">
                ...
                <div className="mt-3 flex items-center justify-between gap-3 border-t border-hairline/30 pt-3">
                  <div>
                    <div className="text-[12px] text-ink">Allow full PWA management</div>
                    <div className="mt-0.5 text-[11px] text-ink-secondary">Settings, imports, deletion, and host-affecting actions.</div>
                  </div>
                  <button
                    role="switch"
                    aria-checked={device.administrationAccess === true}
                    aria-label={`Administration access for ${device.name}`}
                    disabled={c.busy}
                    onClick={() =>
                      void c.act((companion) =>
                        companion.administration(device.id, device.administrationAccess !== true),
                      )
                    }
                    className={cnSwitch(device.administrationAccess === true)}
                  >
                    <span className={cnKnob(device.administrationAccess === true)} />
                  </button>
                </div>
                <div className="mt-3 flex items-center justify-between gap-3 border-t border-hairline/30 pt-3">
                  <div>
