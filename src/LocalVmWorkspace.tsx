...
function LocalVmPane({
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      if (!bridge) {
        setError("The two-desktop workspace requires the Pip desktop app.");
        return;
      }
      try {
        ...
        setError(
          cause instanceof Error && cause.message === "layout-unavailable"
            ...
            : "Pip could not connect this Local VM viewer.",
        );
      }
    };
  ...
  const updateLayout = useCallback(() => {
    ...
    void bridge
      ...
      .catch(() => setError("Pip could not position this Local VM viewer."));
...
export function LocalVmWorkspace({
  ...
  const handBack = useCallback(async () => {
    ...
    try {
      ...
      setControlError("Pip could not hand control back. The workspace stayed open.");
      return false;
    } finally {
