...
export interface BotPatchPartitions {
  profile: BotUpdatePatch;
  desktop: BotUpdatePatch;
}

const PAIRED_PROFILE_FIELDS = [
  "name",
  "title",
  "description",
  "notifications",
  ...
  "voice",
  "speakReplies",
] as const satisfies readonly (keyof BotUpdatePatch)[];

/**
 * Keep paired-device writes on the deliberately narrow profile boundary.
 * Desktop-only controls stay on the broad local route, so the companion can
 * reject them without also blocking harmless character customization.
 */
export function partitionBotUpdatePatch(patch: BotUpdatePatch): BotPatchPartitions {
  const profile: BotUpdatePatch = {};
  const desktop: BotUpdatePatch = { ...patch };
  for (const field of PAIRED_PROFILE_FIELDS) {
    if (field in patch) {
      Object.assign(profile, { [field]: patch[field] });
      delete desktop[field];
    }
  }
