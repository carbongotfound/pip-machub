...
import type { MascotAccessory, MascotBody, MascotEyes } from "../../shared/mascot-profile";
  ...
  switch (action.type) {
    ...
    case "updateBot": {
      const mascotChanged =
        ...
        Object.prototype.hasOwnProperty.call(action.patch, "mascotEyes") ||
        Object.prototype.hasOwnProperty.call(action.patch, "mascotAccessory");
    ...
    () =>
      createBotPatchQueue({
        send: async (botId, patch, signal) => {
          const { profile, desktop } = partitionBotUpdatePatch(patch);
          let saved: BotAnnouncement | null = null;
          if (Object.keys(profile).length > 0) {
            const result: { bot: BotAnnouncement } = await api(`/api/bots/${botId}/profile`, {
              method: "PATCH",
          ...
          if (!saved) throw new Error("Bot update had no fields to save");
          return saved;
        },
